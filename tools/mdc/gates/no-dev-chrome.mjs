 ...
 *   MDC_ASSURE_EVIDENCE_T5 = <dir>   →  read that directory, and bind to the CURRENT artifact record.
 ...
 */
const ASSURE_DIR = process.env.MDC_ASSURE_EVIDENCE_T5 || null;
const EVIDENCE_DIR = ASSURE_DIR || join(CAMPAIGN_DIR, 'evidence', 'external', 'T5');
...
export const run = async () => {
  ...
  const artifact = bindToRecordedArtifact({ campaignDir: CAMPAIGN_DIR, apkPath: APK, hostSha, stage: ASSURE_DIR ? 'CURRENT' : 'STAGE-2' });
