#!/usr/bin/env node
/**
 * P4 GATE RUNNER — `npm run p4:gate -- <name>`.
 *
 * The same three rules the P2 and P3 runners enforce, carried because they are how this project
 * keeps a green line honest:
 *
 *   1. THE GATE POPULATION IS DERIVED FROM tools/p4/gates/*.mjs, never from a list kept here.
 ...
 *
 * ONE RULE THIS RUNNER ADDS, AND WHY. P4 is the first phase whose criteria are about what a person
 * sees, so contract §2 rule 9 says a screen assertion is measured against the RENDERED surface and
 * not the source. A gate may therefore declare `MEASURES: 'render' | 'source' | 'artifact'`, and it
 * is recorded in the report so that a criterion about a screen can be audited for whether the thing
 * that checked it ever rendered anything. A test that greps a component for a string proves the
 * string is in the file.
 *
 * Trailing flags such as `--app` are ignored: the ledger passes them so it knows which repository
 * to run in, and they are not gate names.
 *
 * Usage:
 *   npm run p4:gate -- <name>      run one gate
 *   npm run p4:gate -- --list      list every gate on disk and its state
 *
...
export const requiredGates = () => {
  if (!existsSync(REQUIRED_PATH)) return { error: 'tools/p4/required-gates.json is missing — without it this ladder can only report on the gates it happens to have, which is not what the contract asks' };
  try {
...
export const runGate = async (name) => {
  ...
  if (!mod) return { name, ok: false, unknown: true, message: 'no gate module tools/p4/gates/' + name + '.mjs' };
  if (typeof mod.run !== 'function') {
    return { name, ok: false, message: 'tools/p4/gates/' + name + '.mjs exports no run()' };
  }
  if (!mod.SENTINEL) {
    return { name, ok: false, message: 'tools/p4/gates/' + name + '.mjs declares no SENTINEL. A gate without a positive sentinel cannot be decided on printed output.' };
  }
  let r;
  try {
  ...
  return { name, sentinel: mod.SENTINEL, criteria: mod.CRITERIA ?? [], measures: mod.MEASURES ?? null, ...r };
};
...
if (isMain) {
  ...
  if (args.includes('--list') || args.length === 0) {
    ...
    console.log('');
    console.log('  P4 GATES — ' + names.length + ' on disk, derived from tools/p4/gates/*.mjs');
    ...
    if (error) {
      ...
      if (missing.length) {
        ...
        for (const n of missing) {
          ...
          console.log('      ' + n.padEnd(26) + (g?.criteria ?? []).join(',').padEnd(10) + (g?.sentinel ?? ''));
        }
      }
    }
    console.log('');
    console.log('P4-GATE OK — ' + names.length + ' gate(s) listed');
    process.exit(0);
  }
  ...
  if (!names.includes(name)) {
    ...
    console.log('');
    console.log('P4-GATE FAILED — unknown gate "' + name + '".');
    console.log('  There is no tools/p4/gates/' + name + '.mjs. This is a hard failure on purpose:');
  ...
  if (r.notImplemented) {
    console.log('P4-GATE NOT IMPLEMENTED — ' + name + (r.message ? ': ' + r.message : ''));
  ...
  console.log('P4-GATE FAILED — ' + name + ': ' + (r.message ?? 'no reason given'));
