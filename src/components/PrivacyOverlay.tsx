...
// THE ASSET LIVES IN VERSION CONTROL, not in prebuild output. This used to require the image out
// of `android/app/src/main/res/drawable-xxxhdpi/`, which `.gitignore` line 13 excludes because
// `android/` is generated by `expo prebuild`. It resolved on the machine that had run prebuild and
// on no other: a fresh clone has no `android/` at all, so the privacy overlay — the thing that
// covers the screen when the app is backgrounded — could not resolve its own image.
const SPLASH_IMAGE: ImageSourcePropType = require('../assets/splashscreen_logo.png');
